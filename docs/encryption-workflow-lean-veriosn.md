...
sequenceDiagram
    ...
    Crypto->>Crypto: Validate sessionJwt signature (what is publickey to validate) , expiry date/time (401 plaintext on failure)
    ...
    alt SEK not found / db error
        ...
        alt not authenticated
            ...
            Kong->>Crypto: POST /pds/internal/encrypt-response<br/>{sessionId, plaintextBodyBase64}
            Crypto->>Crypto: build response envelope, signing with pdsSignKeyPair(PDSprivateKey?) from Vault,<br/>JWE-encrypt with SEK
...
sequenceDiagram
    ...
    alt SEK not found / db error
        ...
        alt inactive (revoked/logged out)
            ...
            Crypto->>Crypto: build 0x00 response envelope, signing with pdsSignKeyPair(PDSprivateKey?) from Vault,<br/>JWE-encrypt with SEK
            Crypto-->>Kong: {responseJwe}
